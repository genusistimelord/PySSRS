...
class SSRS():
    ...
    def __init__(self, ReportService, ReportExecution, user, key_password, domain=None, verbose=True):
        ...
        try:
            self.ServiceClient = Client(
                ReportService, transport=suds_requests.RequestsTransport(servsession))
            self.ExecutionClient = Client(
                ReportExecution, transport=suds_requests.RequestsTransport(execsession))
    ...
    def ListServiceMethods(self):
        ...
        try:
            list_of_methods = [
                method for method in self.ServiceClient.wsdl.services[0].ports[0].methods]
    ...
    def ListExecutionMethods(self):
        ...
        try:
            list_of_methods = [
                method for method in self.ExecutionClient.wsdl.services[0].ports[0].methods]
            return list_of_methods
    ...
    def GetParameters(self, path):
        ...
        '''
        try:
            it = self.ServiceClient.service.GetItemParameters(
    ...
    def RenderReport(self, LoadedReport, format, **parameters):
        ...
        available_formats = ['XML', 'NULL', 'CSV', 'IMAGE',
                             'PDF', 'HTML4.0', 'HTML3.2', 'MHTML', 'EXCEL', 'Word']
        ...
        xml = schemas.xml_Render_Report.format(
            LoadedReport.ExecutionID, format)
